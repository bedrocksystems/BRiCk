...
printSimpleContext(const DeclContext* dc, CoqPrinter& print, ClangPrinter& cprint, size_t remaining = 0) {
    ...
    if (dc == nullptr or dc->isTranslationUnit()) {
        ...
        auto compound = printSimpleContext(parent, print, cprint, remaining + 1);
...
printQualifiedName(const NamedDecl& decl, CoqPrinter& print, ClangPrinter& cprint) {
    print.output() << '\"';
...
// more invasive.
static fmt::Formatter&
printDtorName(const CXXRecordDecl& decl, CoqPrinter& print, ClangPrinter& cprint) {
...
classifyContext(const DeclContext& ctx, ClangPrinter& cprint) {
    ...
    case Decl::Kind::Enum:
        return cast<EnumDecl>(ctx).isScoped()
            ? ContextKind::Scope
...
getNonIgnorableAncestor(const Decl& decl, ClangPrinter& cprint) {
    ...
    auto parent = [&](const DeclContext* ctx) {
        if (auto p = ctx->getParent()) {
            ...
            fatal("declaration context outside any translation unit", loc::of(ctx));
    };
...
printTemplateParameters(const Decl& decl, CoqPrinter& print,
    ...
    if (collectParameterLists(decl, cprint.getContext(), lists)) {
        ...
        for (auto [params, loc] : lists)
            for (auto param : params->asArray())
                printTemplateParameter(param, print, cprint, loc, as_arg) << fmt::cons;
        return print.output();
    } else
...
static fmt::Formatter&
printName(const Decl&, CoqPrinter&, ClangPrinter&);
...
fmt::Formatter&
ClangPrinter::printTemplateParameters(const Decl& decl, CoqPrinter& print, bool as_arg) {
    if (trace(Trace::Name)) trace("printTemplateParameters", loc::of(decl));
    return structured::printTemplateParameters(decl, print, *this, as_arg);
}

fmt::Formatter&
ClangPrinter::printTemplateArgumentList(const TemplateArgumentList& args, CoqPrinter& print, loc::loc loc) {
    if (trace(Trace::Name)) trace("printTemplateArgumentList", loc);
    return structured::printTemplateArgumentList(args, print, *this, loc);
}

fmt::Formatter&
ClangPrinter::printTemplateArgumentList(ArrayRef<TemplateArgument> args, CoqPrinter& print) {
    if (trace(Trace::Name)) trace("printTemplateArgumentList", loc::none);
    auto& cprint = *this;
...
fmt::Formatter&
ClangPrinter::printTemplateArgumentList(ArrayRef<TemplateArgumentLoc> args, CoqPrinter& print) {
    if (trace(Trace::Name)) trace("printTemplateArgumentList", loc::none);
    auto& cprint = *this;
    return print.list(args, [&](auto& arg) {
        structured::printTemplateArgument(arg.getArgument(), print, cprint, loc::of(arg));
    });
}

fmt::Formatter&
ClangPrinter::printTemplateArguments(const Decl& decl, CoqPrinter& print) {
    if (trace(Trace::Name)) trace("printTemplateArguments", loc::of(decl));
    return structured::printTemplateArguments(decl, print, *this);
}

fmt::Formatter&
ClangPrinter::printNameComment(const Decl& decl, CoqPrinter& print) {
    if (trace(Trace::Name)) trace("printNameComment", loc::of(decl));
...
template<typename T>
T&
deref(CoqPrinter& print, ClangPrinter& cprint, const std::string whence, T* p, loc::loc loc) {
    if (! p) {
...
ClangPrinter::printStructuredName(const Decl& decl, CoqPrinter& print) {
    assert(print.ast2());
    if (trace(Trace::Name)) trace("printStructuredName", loc::of(decl));
    return structured::printName(decl, print, *this);
}

fmt::Formatter&
ClangPrinter::printUnqualifiedName(const NamedDecl& decl, CoqPrinter& print) {
    if (trace(Trace::Name)) trace("printUnqualifiedName", loc::of(decl));
...
fmt::Formatter&
ClangPrinter::printUnqualifiedName(const NamedDecl* p, CoqPrinter& print, loc::loc loc) {
...
ClangPrinter::printMangledName(const Decl& decl, CoqPrinter& print) {
    if (trace(Trace::Name)) trace("printMangledName", loc::of(decl));
    return mangled::printName(decl, print, *this);
}

fmt::Formatter&
ClangPrinter::printMangledTypeName(const TypeDecl& decl, CoqPrinter& print) {
    if (trace(Trace::Name)) trace("printMangledTypeName", loc::of(decl));
    return mangled::printTypeName(decl, print, *this);
}

fmt::Formatter&
ClangPrinter::printMangledTypeName(const TypeDecl* p, CoqPrinter& print, loc::loc loc) {
...
ClangPrinter::printMangledObjName(const ValueDecl& decl, CoqPrinter& print) {
    if (trace(Trace::Name)) trace("printMangledObjName", loc::of(decl));
    return mangled::printObjName(decl, print, *this);
}

fmt::Formatter&
ClangPrinter::printMangledObjName(const ValueDecl* p, CoqPrinter& print, loc::loc loc) {
...
ClangPrinter::printTypeName(const TypeDecl& decl, CoqPrinter& print) {
    if (trace(Trace::Name)) trace("printTypeName", loc::of(decl));
    if (print.ast2())
        return structured::printName(decl, print, *this);
    else
        return mangled::printTypeName(decl, print, *this);
...
fmt::Formatter&
ClangPrinter::printTypeName(const TypeDecl* p, CoqPrinter& print, loc::loc loc) {
...
ClangPrinter::printDtorName(const CXXRecordDecl& decl, CoqPrinter& print) {
    if (trace(Trace::Name)) trace("printDtorName", loc::of(decl));
    if (print.ast2())
...
ClangPrinter::printObjName(const ValueDecl& decl, CoqPrinter& print) {
    if (trace(Trace::Name)) trace("printObjName", loc::of(decl));
    if (print.ast2())
        return structured::printName(decl, print, *this);
    else
        return mangled::printObjName(decl, print, *this);
...
fmt::Formatter&
ClangPrinter::printObjName(const ValueDecl* p, CoqPrinter& print, loc::loc loc) {
