...
class PrintSpec : public ConstDeclVisitorArgs<PrintSpec, void, CoqPrinter &, ClangPrinter &, clang::comments::FullComment&> {
private:
	llvm::StringRef get_text(SourceRange range) const {
		auto len = sm_.getCharacterData(range.getEnd()) - sm_.getCharacterData(range.getBegin());
		return StringRef(sm_.getCharacterData(range.getBegin()), len+1);
	}

	void write_paragraph(CoqPrinter &print, comments::ParagraphComment *comment) const {
	...
	comments::ParagraphComment* get_tag(clang::comments::FullComment& comment, const char* tag) const {
	...
	void print_function_spec(CoqPrinter &print, ClangPrinter &cprint, clang::comments::FullComment& comment) const {
		// print the \with blocks
		{
			bool started = false;
			for (auto b : comment.getBlocks()) {
				if (auto pcc = dyn_cast<comments::ParagraphComment>(b)) {
					auto sr = get_text(pcc->getSourceRange()).trim(' ').trim('\n').trim(' ');
	...
	void print_arguments(const FunctionDecl *decl, CoqPrinter &print, ClangPrinter &cprint, clang::comments::FullComment& cmt, bool with_this) {
	...
	void print_parameter_types(const FunctionDecl* decl, CoqPrinter &print, ClangPrinter &cprint) {
		print.begin_list();
		for (auto i : decl->parameters()) {
...
public:
	...
	void VisitCXXMethodDecl(const CXXMethodDecl *decl, CoqPrinter &print, ClangPrinter &cprint, clang::comments::FullComment& cmt) {
	...
	void VisitFunctionDecl(const FunctionDecl *decl, CoqPrinter &print, ClangPrinter &cprint, clang::comments::FullComment& cmt) {
		if (auto spec_para = get_tag(cmt, "\\spec")) {
			write_paragraph(print, spec_para);
			return;
		}
	...
	void VisitCXXConstructorDecl(const CXXConstructorDecl *decl, CoqPrinter &print, ClangPrinter &cprint, clang::comments::FullComment& cmt) {
		if (auto spec_para = get_tag(cmt, "\\spec")) {
			write_paragraph(print, spec_para);
			return;
		}
	...
	void VisitCXXDestructorDecl(const CXXDestructorDecl *decl, CoqPrinter &print, ClangPrinter &cprint, clang::comments::FullComment& cmt) {
		if (auto spec_para = get_tag(cmt, "\\spec")) {
			write_paragraph(print, spec_para);
			return;
		}
...
public:
	PrintSpec(ASTContext& ctxt):ctxt_(ctxt),sm_(ctxt.getSourceManager()),diag_(ctxt.getDiagnostics()) {}
...
static void begin_decl(const NamedDecl *decl, CoqPrinter& print, ClangPrinter& cprint) {
...
static void end_decl(const NamedDecl *, CoqPrinter& print, ClangPrinter&) {
	print.output() << fmt::outdent;
	print.end_record();
	print.output() << fmt::outdent << "." << fmt::line;
}
...
write_globals(::Module& mod, CoqPrinter& print, ClangPrinter& cprint) {
	...
	for (auto i : mod.definitions()) {
		...
		if (const FieldDecl* fd = dyn_cast<FieldDecl>(def)) {
			print.output() << "Notation \"'" << fd->getNameAsString() << "'\" :=" << fmt::nbsp;
			...
			print.output() << "Notation \"'" << class_name << "'\" :=" << fmt::nbsp;
			cprint.printGlobalName(def, print);
			print.output() << "%string (in custom cppglobal at level 0)." << fmt::line;

			for (auto fd : rd->fields()) {
				print.output() << "Notation \"'" << class_name << "::" << fd->getNameAsString() << "'\" :=" << fmt::nbsp;
				cprint.printField(fd, print);
				print.output() << " (in custom cppglobal at level 0)." << fmt::line;
			}
		}
...
void
write_spec(::Module* mod, const SpecCollector &specs, const clang::TranslationUnitDecl* tu, Filter& filter, fmt::Formatter& output) {
	...
	print.output()
        << "(*" << fmt::line
				<< " * Specifications extracted from " << ctxt.getSourceManager().getFileEntryForID(ctxt.getSourceManager().getMainFileID())->getName() << fmt::line
				<< " *)" << fmt::line
				<< fmt::line
  ...
  for (auto c : ctxt.getRawCommentList().getComments()) {
		if (!source.isIncluded(c->getBeginLoc())) {
			if (c->isAttached()) {
				...
				output << "(* BEGIN_SOURCE(" <<	comment->getBeginLoc().printToString(ctxt.getSourceManager()) << ") *)" << fmt::line;
				begin_decl(decl, print, cprint);
				printer.Visit(decl, print, cprint, *comment);
				end_decl(decl, print, cprint);
				output << "(* END_SOURCE(" <<	comment->getEndLoc().printToString(ctxt.getSourceManager()) << ") *)" << fmt::line << fmt::line;
				...
				if (c->getKind() == RawComment::RCK_Qt || c->getKind() == RawComment::RCK_BCPLExcl) {
					auto text = c->getRawText(ctxt.getSourceManager());

					if (text.startswith("/*!coq")) {
						output << "(* BEGIN_SOURCE(" <<	c->getBeginLoc().printToString(ctxt.getSourceManager()) << ") *)" << fmt::line;
						output << text.substr(7).drop_back(2).trim() << fmt::line;
						output << "(* END_SOURCE(" <<	c->getEndLoc().printToString(ctxt.getSourceManager()) << ") *)" << fmt::line << fmt::line;
					}
				}
			}
		}
	...
	print.output()
        << fmt::line
        << "Definition internal_spec : signature :=" << fmt::indent << fmt::line
        << "make_signature" << fmt::nbsp;
  print.begin_list();
  ...
  print.output()
        << fmt::line
        << "Definition public_spec : signature :=" << fmt::indent << fmt::line
        << "make_signature" << fmt::nbsp;
  print.begin_list();
