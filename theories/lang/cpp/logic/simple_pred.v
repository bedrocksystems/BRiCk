...
Local Lemma length__Z_to_bytes_le {σ} n v :
  length (_Z_to_bytes_le (σ:=σ) n v) = n.
Proof. by rewrite /_Z_to_bytes_le fmap_length seq_length. Qed.

Local Lemma length__Z_to_bytes {σ} n v :
  length (_Z_to_bytes (σ:=σ) n v) = n.
Proof.
  rewrite /_Z_to_bytes.
  by case: (byte_order _); rewrite ?rev_length length__Z_to_bytes_le.
Qed.
...
Module SimpleCPP_BASE <: CPP_LOGIC_CLASS.
  ...
  Definition Z_to_bytes {σ:genv} (n : nat) (v : Z) : list runtime_val :=
    Rval <$> _Z_to_bytes (σ:=σ) n v.

  Lemma length_Z_to_bytes {σ} n v : length (Z_to_bytes (σ:=σ) n v) = n.
