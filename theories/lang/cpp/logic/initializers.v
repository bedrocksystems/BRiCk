...
Section with_resolve.
  ...
  (**
  [default_initialize_array default_init ty len p Q] initializes an
  array of type [ty[len]] at pointer [p].
  ...
  [ERROR].
  *)
  Definition default_initialize_array
      (default_initialize : type -> ptr -> (FreeTemps -> epred) -> mpred)
      (ty : type) (len : N) (p : ptr) (Q : FreeTemps -> epred) : mpred :=
    let folder i PP :=
      default_initialize ty (p ,, o_sub _ ty (Z.of_N i)) (fun free' => interp free' PP)
    in
  ...
  Lemma default_initialize_array_frame di ty sz Q Q' (p : ptr) :
    (∀ p Q Q', (Forall f, Q f -* Q' f) |-- di ty p Q -* di ty p Q') ->
    (Forall f, Q f -* Q' f)
  ...
  Proof.
    ...
    - iIntros "F". iApply IHty.
      iIntros (?). iApply interp_frame. iApply (IHl with "F").
  Qed.

  (**
  [default_initialize ty p Q] default initializes the memory at [p]
  according to the type [ty].

  NOTE this assumes that the underlying memory has already been
  given to the C++ abstract machine.
  ...
  and [default_initialize] corresponds to [default-initialization]
  as described above.
  *)
  ...
  *)
  Definition wp_initialize (qty : type) (addr : ptr) (init : Expr) (Q : FreeTemps -> mpred) : mpred :=
...
Section mono_frames.
  ...
  requires some additional side-conditions on paths that we can't
  prove right now. So, for the time being, we prove [_frame] lemmas
  without [genv] weakening.
  *)
