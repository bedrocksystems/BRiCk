...
Section with_Σ.
  ...
  Axiom code_at_ok : forall (f : Func) p,
      ...
      |-- Forall ls Q, wp_func (resolve:=σ) f ls Q -*
                       fspec (Σ:=Σ) σ.(genv_tu).(globals) (type_of_value (Ofunction f)) p ls Q.

  Axiom method_at_ok : forall (m : Method) p,
      method_at σ m p
      |-- Forall ls Q, wp_method (resolve:=σ) m ls Q -*
                       fspec (Σ:=Σ) σ.(genv_tu).(globals) (type_of_value (Omethod m)) p ls Q.

  Axiom ctor_at_ok : forall (c : Ctor) p,
      ctor_at σ c p
      |-- Forall ls Q, wp_ctor (resolve:=σ) c ls Q -*
                       fspec σ.(genv_tu).(globals) (type_of_value (Oconstructor c)) p ls Q.

  Axiom dtor_at_ok : forall (d : Dtor) p,
      dtor_at σ d p
      |-- Forall ls Q, wp_dtor (resolve:=σ) d ls Q -*
                       fspec σ.(genv_tu).(globals) (type_of_value (Odestructor d)) p ls Q.
  (* END COMPILE SNIPPET *)
