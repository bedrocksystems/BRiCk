...
recoverTemplate(const clang::Decl&);
...
recoverPattern(const clang::Decl&);
...
public:
    ...
    ClangPrinter(clang::CompilerInstance* compiler,
...
public:
    ...
    fmt::Formatter& printTypeName(const clang::TypeDecl& decl, CoqPrinter& print);
    fmt::Formatter& printTypeName(const clang::TypeDecl* decl, CoqPrinter& print, loc::loc);
    fmt::Formatter& printDtorName(const clang::CXXRecordDecl&, CoqPrinter&);
    fmt::Formatter& printObjName(const clang::ValueDecl& decl, CoqPrinter& print);
    fmt::Formatter& printObjName(const clang::ValueDecl* decl, CoqPrinter& print, loc::loc);
    ...
    fmt::Formatter& printTemplateParameters(const clang::Decl&, CoqPrinter&, bool as_arg = false);

    fmt::Formatter& printTemplateArgumentList(const clang::TemplateArgumentList&, CoqPrinter&, loc::loc);
    fmt::Formatter& printTemplateArgumentList(llvm::ArrayRef<clang::TemplateArgument>, CoqPrinter&);
    fmt::Formatter& printTemplateArgumentList(llvm::ArrayRef<clang::TemplateArgumentLoc>, CoqPrinter&);
    ...
    fmt::Formatter& printStructuredName(const clang::Decl&, CoqPrinter&); // : name
    fmt::Formatter& printMangledName(const clang::Decl&, CoqPrinter&); // : bs
    fmt::Formatter& printMangledTypeName(const clang::TypeDecl&, CoqPrinter&); // : bs
    fmt::Formatter& printMangledTypeName(const clang::TypeDecl*, CoqPrinter&, loc::loc);
    fmt::Formatter& printMangledObjName(const clang::ValueDecl&, CoqPrinter&); // : bs
    fmt::Formatter& printMangledObjName(const clang::ValueDecl*, CoqPrinter&, loc::loc);
    fmt::Formatter& printUnqualifiedName(const clang::NamedDecl&, CoqPrinter&); // : bs
    fmt::Formatter& printUnqualifiedName(const clang::NamedDecl*, CoqPrinter&, loc::loc);

    // TODO: Adjust and use in the structured name printer
    fmt::Formatter& printNameForAnonTemplateParam(unsigned depth, unsigned index, CoqPrinter& print, loc::loc);

    fmt::Formatter& printParamName(const clang::ParmVarDecl* d, CoqPrinter& print);

    fmt::Formatter& printOverloadableOperator(clang::OverloadedOperatorKind, CoqPrinter&, loc::loc);

    // Types

    fmt::Formatter& printQualType(const clang::QualType& qt, CoqPrinter& print, loc::loc loc);
    fmt::Formatter& printQualTypeOption(const clang::QualType& qt, CoqPrinter& print, loc::loc loc);

    fmt::Formatter& printType(const clang::Type&, CoqPrinter&);
    fmt::Formatter& printType(const clang::Type* t, CoqPrinter& print, loc::loc);

    fmt::Formatter& printQualifier(bool is_const, bool is_volatile,
                                  CoqPrinter& print) const;
    fmt::Formatter& printQualifier(const clang::QualType& qt, CoqPrinter& print) const;
    ...
    fmt::Formatter& printExpr(const clang::Expr* d, CoqPrinter& print, OpaqueNames& li);
    fmt::Formatter& printExpr(const clang::Expr* d, CoqPrinter& print);

    fmt::Formatter& printValueDeclExpr(const clang::ValueDecl*, CoqPrinter& print, OpaqueNames&);
    fmt::Formatter& printValueDeclExpr(const clang::ValueDecl*, CoqPrinter& print);
