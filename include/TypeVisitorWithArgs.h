...
#define DISPATCH(CLASS) \
  return static_cast<ImplClass*>(this)-> \
           Visit##CLASS(static_cast<const CLASS*>(T), std::forward<ParamTys>(P)...)
...
public:
  ...
  RetTy Visit(const Type *T, ParamTys... P) {
...
#include "clang/AST/TypeNodes.def"
  ...
  // back on superclass.
#define TYPE(CLASS, PARENT) RetTy Visit##CLASS##Type(const CLASS##Type *T, ParamTys... P) { \
  DISPATCH(PARENT);                                                          \
}
#include "clang/AST/TypeNodes.def"
  ...
  /// for some type class.
  RetTy VisitType(const Type*, ParamTys... P) { return RetTy(); }
};
